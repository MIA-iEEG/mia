...
for iSubj = 1:length(subjectNames)
    % Prefer files tagged "MIA". If none are found for this subject and
    % condition, fall back to the same Brainstorm query without any tag.
    sFiles = select_subject_files(subjectNames{iSubj}, Condition);
...
function sFiles = select_subject_files(subjectName, conditionName)
% select_subject_files:
%   1) Try to retrieve files for the subject/condition with tag = 'MIA'
%   2) If nothing is found, retry the same selection without filtering on tag

% Shared Brainstorm selection arguments used in both attempts.
baseArgs = { ...
...
% First pass: keep the original behavior and only select files tagged MIA.
sFiles = bst_process('CallProcess', 'process_select_files_data', [], [], ...
    baseArgs{:}, ...
    'tag', 'MIA');

if isempty(sFiles)
    % Second pass: if no MIA-tagged files exist, select the same files
    % without applying the MIA tag filter.
    try
        sFiles = bst_process('CallProcess', 'process_select_files_data', [], [], ...
            baseArgs{:});
    catch
        % Some Brainstorm versions expect the tag argument to be present.
        sFiles = bst_process('CallProcess', 'process_select_files_data', [], [], ...
            baseArgs{:}, ...
            'tag', '');
    end
